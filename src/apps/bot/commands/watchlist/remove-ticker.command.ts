...
export const data = new SlashCommandBuilder()
  ...
  .setDescription('Remove a ticker from the market analysis watchlist')
...
export async function execute(
  ...
  try {
    ...
    await tickerManagementModule?.removeTickerUseCase.execute({ symbol: normalizedSymbol });

    await interaction.reply(
      `✅ Successfully removed **${normalizedSymbol}** from the watchlist!`
    );
  } catch (error) {
    const message =
